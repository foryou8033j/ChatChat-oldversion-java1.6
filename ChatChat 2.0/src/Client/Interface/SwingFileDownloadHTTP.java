...
public class SwingFileDownloadHTTP extends JFrame implements PropertyChangeListener
{
	...
	private JFilePicker filePicker = new JFilePicker("경로 지정: ", "Browse...");
	private JButton buttonDownload = new JButton("내려받기");
	private JLabel labelFileName = new JLabel("파일 이름: ");
	private JTextField fieldFileName = new JTextField(20);
	private JLabel labelFileSize = new JLabel("파일 크기 (bytes): ");
	private JTextField fieldFileSize = new JTextField(20);
	private JLabel labelProgress = new JLabel("Progress:");
	private JProgressBar progressBar = new JProgressBar(0, 100);
	private boolean isUpdate = false;
	
	public SwingFileDownloadHTTP(String url, boolean isUpdate)
	{
		super("파일 내려받기");
		...
		fieldURL.setText("http://" + url + "/" + MaintenanceData.clientFileName);
		...
		if (isUpdate) setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
	}
	...
	private void buttonDownloadActionPerformed(ActionEvent event)
	{
		...
		{
			JOptionPane.showMessageDialog(this, "다운로드할 원격지 주소를 입력하세요.", "오류", JOptionPane.ERROR_MESSAGE);
			fieldURL.requestFocus();
			return;
		}
		if (saveDir.equals(""))
		{
			JOptionPane.showMessageDialog(this, "저장할 파일 경로를 지정하세요.", "오류", JOptionPane.ERROR_MESSAGE);
			return;
		}
		try
		{
